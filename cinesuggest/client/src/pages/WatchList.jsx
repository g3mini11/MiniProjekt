...
import { removeWatchlist } from "../redux/features/userSlice";

const WatchlistItem = ({ media, onRemoved }) => {
  ...
  const onRemove = async () => {
    ...
    if (response) {
      ...
      dispatch(removeWatchlist({ mediaId: media.mediaId }));
      onRemoved(media.id);
    }
  };
...
const WatchlistList = () => {
  ...
  return (
    <Box sx={{ ...uiConfigs.style.mainContent }}>
      <Container header={`Your watchlists (${count})`}>
        <Grid container spacing={1} sx={{ marginRight: "-8px!important" }}>
          {filteredMedias.map((media, index) => (
            <Grid item xs={6} sm={4} md={3} key={index}>
              <WatchlistItem media={media} onRemoved={onRemoved} />
            </Grid>
          ))}
        </Grid>
